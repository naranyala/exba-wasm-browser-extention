...
function checkFile(filePath, description) {
  const absolutePath = path.isAbsolute(filePath) ? filePath : path.join(distDir, filePath);
  if (!fs.existsSync(absolutePath)) {
    console.error(`${RED}✗ Missing File:${NC} ${description} - Expected at: ${absolutePath}`);
    errors++;
    return false;
  }
...
if (!fs.existsSync(distDir)) {
  console.error(`${RED}✗ Error:${NC} Output directory does not exist at ${distDir}`);
  process.exit(1);
}

// 2. Verify manifest.json exists
if (!checkFile('manifest.json', 'manifest.json')) {
  console.error(`${RED}✗ Critical Error: manifest.json is missing. Cannot proceed with validation.${NC}`);
  process.exit(1);
}
...
try {
  ...
  console.error(`${RED}✗ Parse Error:${NC} Failed to parse manifest.json: ${e.message}`);
  process.exit(1);
}

// 4. Verify Manifest Version
if (manifest.manifest_version !== 3) {
  console.error(`${RED}✗ Error:${NC} manifest_version must be 3 (found: ${manifest.manifest_version})`);
  errors++;
} else {
...
if (manifest.background) {
  if (manifest.background.service_worker) {
    checkFile(manifest.background.service_worker, `Background service worker (${manifest.background.service_worker})`);
  } else {
    console.error(`${RED}✗ Error:${NC} background property specified but service_worker is missing`);
    errors++;
  }
}
...
if (manifest.side_panel && manifest.side_panel.default_path) {
  checkFile(manifest.side_panel.default_path, `Side Panel default path (${manifest.side_panel.default_path})`);
}
...
checkFile('wasm/pkg/wasm_unified_core_bg.wasm', 'Wasm compiled binary (wasm/pkg/)');
